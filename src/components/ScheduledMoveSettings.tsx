...
export const ScheduledMoveSettings: React.FC<Props> = ({ accounts, schedules, onChange }) => {
  ...
  return <section className="account-settings">
    ...
    {editingId && <div className="account-editor">
      ...
      <label>金額<input type="number" inputMode="numeric" min="1" step="1" value={revision.amount} onChange={(event) => setRevision({ ...revision, amount: Number(event.target.value) })} /></label>
      ...
      {revision.frequency !== "daily" && <label>{revision.frequency === "monthly" ? "実行日（1〜31）" : "曜日"}{revision.frequency === "monthly" ? <input type="number" inputMode="numeric" min="1" max="31" value={revision.executionDay} onChange={(event) => setRevision({ ...revision, executionDay: Math.max(1, Math.min(31, Number(event.target.value))) })} /> : <select value={revision.executionDay} onChange={(event) => setRevision({ ...revision, executionDay: Number(event.target.value) })}>{["日","月","火","水","木","金","土"].map((day, index) => <option key={day} value={index}>{day}</option>)}</select>}</label>}
